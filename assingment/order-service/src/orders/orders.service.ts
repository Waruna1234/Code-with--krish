...
export class OrdersService implements OnModuleInit {
  ...
  async create(createOrderDto: createOrderDto): Promise<any> {
    ...
    for(const item of items){
      const lockKey = `waruna.product:${item.productId}:lock`
    ...
    this.producer.send({
      topic:'waruna-order.create',
      messages:[{value: JSON.stringify({customerId,city,customerName,items})}],
    });
  ...
  async consumeConfermOrders(){
    await this.consumer.subscribe({topic:'waruna.order.inventory.update'})
